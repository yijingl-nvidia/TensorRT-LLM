...
using MoeRunnerArgs = tensorrt_llm::kernels::trtllmGenFp8BlockScaleMoe::MoE::MoERunnerArgs;
using MoeRunnerType = tensorrt_llm::kernels::trtllmGenFp8BlockScaleMoe::MoE::Runner;
using MoeWorkspace = tensorrt_llm::kernels::trtllmGenFp8BlockScaleMoe::MoE::MoEWorkspace;
using tensorrt_llm::kernels::trtllmGenFp8BlockScaleMoe::computeSelectedTileN;

namespace
{

MoeRunnerArgs makeFp8BlockScaleMoeArgs(at::Tensor const& hidden_states, int64_t const num_experts, int64_t const top_k,
    ...
    std::optional<double> const routed_scaling_factor)
{
    ...
    args.num_tokens = static_cast<int32_t>(hidden_states.sizes()[0]);
    args.num_experts = static_cast<int32_t>(num_experts);
    args.hidden_size = static_cast<int32_t>(hidden_states.sizes()[1]);
    args.top_k = static_cast<int32_t>(top_k);
    args.n_group = static_cast<int32_t>(n_group.value_or(0));
    args.topk_group = static_cast<int32_t>(topk_group.value_or(0));
    args.local_expert_offset = static_cast<int32_t>(local_expert_offset);
    args.local_num_experts = static_cast<int32_t>(local_num_experts);
    args.routed_scaling_factor = routed_scaling_factor.value_or(1.0);
    args.intermediate_size = static_cast<int32_t>(intermediate_size);
    args.mUseDeepSeekFp8 = true;
    return args;
}

int32_t getMaxNumPaddedTokens(MoeRunnerArgs const& args, int64_t const tile_tokens_dim)
{
    return tensorrt_llm::kernels::trtllmGenFp8BlockScaleMoe::Routing::getMaxPermutedPaddedCount(
        args.num_tokens, args.top_k, args.num_experts, tile_tokens_dim);
}

int32_t getMaxNumPaddedTokensGemm1(MoeRunnerArgs const& args, int64_t const tile_tokens_dim)
{
    int32_t const max_num_padded_tokens = getMaxNumPaddedTokens(args, tile_tokens_dim);
    return tensorrt_llm::kernels::trtllmGenFp8BlockScaleMoe::Routing::maybeGetMinTokenCount(
        max_num_padded_tokens, 2 * args.intermediate_size, btg::dtypeGetNumBits(args.mDtypeElt));
}

int32_t getMaxNumPaddedTokensGemm2(MoeRunnerArgs const& args, int64_t const tile_tokens_dim)
{
    int32_t const max_num_padded_tokens = getMaxNumPaddedTokens(args, tile_tokens_dim);
    return tensorrt_llm::kernels::trtllmGenFp8BlockScaleMoe::Routing::maybeGetMinTokenCount(
...
btg::Dtype getRoutingLogitsDtype(at::optional<at::Tensor> const& routing_logits)
{
    if (routing_logits.has_value() && routing_logits.value().scalar_type() == at::ScalarType::Float)
    {
...
btg::Dtype getRoutingBiasDtype(std::optional<at::Tensor> const& routing_bias)
{
    if (routing_bias.has_value() && routing_bias.value().scalar_type() == at::ScalarType::Float)
    {
        return btg::Dtype::Fp32;
    }
    return btg::Dtype::Bfloat16;
...
public:
    ...
    [[nodiscard]] std::vector<int64_t> resolveTactic(int64_t topK, int64_t hiddenSize, int64_t intermediateSize,
        int64_t numLocalExperts, int64_t numTokens, std::vector<int64_t> tileConfigPair) const
    {
        auto const [tileN, config] = resolveTileConfig(
            std::move(tileConfigPair), topK, hiddenSize, intermediateSize, numLocalExperts, numTokens);
        return {tileN, config};
    }

    [[nodiscard]] std::vector<at::Tensor> runRouting(at::optional<at::Tensor> const& routing_logits,
        std::optional<at::Tensor> const& routing_bias, at::Tensor const& hidden_states, int64_t num_experts,
    ...
    {
        TORCH_CHECK(tensorrt_llm::common::isSM100Family(), "Only SM100f is supported by FP8 block scale MOE");
        TORCH_CHECK(topk_weights.has_value() == topk_ids.has_value(),
            "topk_weights and topk_ids must either both be provided or both be omitted.");
        TORCH_CHECK(routing_logits.has_value() || (topk_weights.has_value() && topk_ids.has_value()),
            "routing_logits or (topk_ids and topk_weights) must be provided.");

        auto const [tileN, config] = resolveTileConfig(std::move(tile_config_pair), top_k, hidden_states.sizes()[1],
            intermediate_size, local_num_experts, hidden_states.sizes()[0]);
        (void) config;

        auto args = makeFp8BlockScaleMoeArgs(hidden_states, num_experts, top_k, n_group, topk_group, intermediate_size,
            local_expert_offset, local_num_experts, routed_scaling_factor);
        auto const routing_bias_dtype = getRoutingBiasDtype(routing_bias);
        args.mDtypeBias = routing_bias_dtype;
        args.routing_logits = routing_logits.has_value() ? routing_logits.value().data_ptr() : nullptr;
        args.routing_bias = routing_bias.has_value() ? routing_bias.value().data_ptr() : nullptr;
        args.topk_weights = topk_weights.has_value() ? topk_weights.value().data_ptr() : nullptr;
        args.topk_ids = topk_ids.has_value() ? static_cast<int32_t*>(topk_ids.value().data_ptr()) : nullptr;

        auto const routing_device
            = routing_logits.has_value() ? routing_logits.value().device() : topk_ids.value().device();
        at::Tensor num_tokens_per_expert
            = at::detail::empty_cuda({num_experts}, at::ScalarType::Int, routing_device, std::nullopt);
        int32_t const max_num_padded_tokens = getMaxNumPaddedTokens(args, tileN);
        at::Tensor total_num_padded_tokens
            = at::empty({}, at::TensorOptions().device(routing_device).dtype(at::ScalarType::Int));
        at::Tensor expanded_idx_to_permuted_idx
            = at::detail::empty_cuda({args.num_tokens * args.top_k}, at::ScalarType::Int, routing_device, std::nullopt);
        at::Tensor permuted_idx_to_token_idx
            = at::detail::empty_cuda({max_num_padded_tokens}, at::ScalarType::Int, routing_device, std::nullopt);
        at::Tensor expert_weights = at::detail::empty_cuda(
            {args.num_tokens, args.top_k}, at::ScalarType::BFloat16, routing_device, std::nullopt);
        at::Tensor expert_indexes
            = at::detail::empty_cuda({args.num_tokens, args.top_k}, at::ScalarType::Int, routing_device, std::nullopt);
        int64_t const size_of_expert_count_histogram = std::max(num_experts * 2, int64_t(256 * 2));
        at::Tensor expert_count_histogram = at::detail::empty_cuda(
            {size_of_expert_count_histogram}, at::ScalarType::Int, routing_device, std::nullopt);

        int32_t const max_num_ctas = tensorrt_llm::kernels::trtllmGenFp8BlockScaleMoe::Routing::getMaxNumCtasInBatchDim(
            args.num_tokens, args.top_k, args.num_experts, tileN);
        at::Tensor cta_idx_xy_to_batch_idx
            = at::detail::empty_cuda({max_num_ctas}, at::ScalarType::Int, routing_device, std::nullopt);
        at::Tensor cta_idx_xy_to_mn_limit
            = at::detail::empty_cuda({max_num_ctas}, at::ScalarType::Int, routing_device, std::nullopt);
        ...
        void* expert_weights_ptr = args.topk_weights ? args.topk_weights : expert_weights.data_ptr();
        tensorrt_llm::kernels::trtllmGenFp8BlockScaleMoe::Routing::Runner routing_runner(tileN);
        auto const& stream = at::cuda::getCurrentCUDAStream(
            routing_logits.has_value() ? routing_logits.value().get_device() : topk_ids.value().get_device());
        routing_runner.run(args.routing_logits, args.routing_bias, args.num_tokens, args.num_experts, args.top_k,
            args.n_group, args.topk_group, args.local_expert_offset, args.local_num_experts, args.routed_scaling_factor,
            ...
            static_cast<RoutingMethodType>(routing_method_type), stream, getRoutingLogitsDtype(routing_logits),
            args.mDtypeBias);

        at::Tensor expert_weights_for_finalize = topk_weights.has_value() ? topk_weights.value() : expert_weights;
        return {expert_indexes, expert_count_histogram, total_num_padded_tokens, expanded_idx_to_permuted_idx,
            permuted_idx_to_token_idx, expert_weights_for_finalize, num_tokens_per_expert, cta_idx_xy_to_batch_idx,
            cta_idx_xy_to_mn_limit, num_non_exiting_ctas};
    }

    [[nodiscard]] std::vector<at::Tensor> runPermuteGemm1(at::Tensor const& hidden_states,
        at::Tensor const& hidden_states_scale, at::Tensor const& gemm1_weights, at::Tensor const& gemm1_weights_scale,
        at::Tensor const& expert_weights, at::Tensor const& permuted_idx_to_token_idx,
        at::Tensor const& total_num_padded_tokens, at::Tensor const& cta_idx_xy_to_batch_idx,
        at::Tensor const& cta_idx_xy_to_mn_limit, at::Tensor const& num_non_exiting_ctas, int64_t num_experts,
        int64_t top_k, int64_t intermediate_size, int64_t local_num_experts, std::vector<int64_t> tile_config_pair)
    {
        auto const [tileN, config] = resolveTileConfig(std::move(tile_config_pair), top_k, hidden_states.sizes()[1],
            intermediate_size, local_num_experts, hidden_states.sizes()[0]);

        auto args = makeFp8BlockScaleMoeArgs(hidden_states, num_experts, top_k, std::nullopt, std::nullopt,
            intermediate_size, 0, local_num_experts, std::nullopt);
        args.hidden_states = hidden_states.data_ptr();
        args.hidden_states_scale = hidden_states_scale.data_ptr<float>();
        args.gemm1_weights = gemm1_weights.data_ptr();
        args.gemm1_weights_scale = gemm1_weights_scale.data_ptr<float>();

        auto const max_num_padded_tokens_gemm1 = getMaxNumPaddedTokensGemm1(args, tileN);
        at::Tensor gemm1_output = at::detail::empty_cuda({max_num_padded_tokens_gemm1, 2 * intermediate_size},
            at::ScalarType::Float8_e4m3fn, hidden_states.device(), std::nullopt);
        at::Tensor gemm1_output_scale
            = at::detail::empty_cuda({2 * intermediate_size / 128, max_num_padded_tokens_gemm1}, at::ScalarType::Float,
        ...
        auto& moe_runner = *mRunners.at(tileN);
        auto const workspace_sizes = moe_runner.getWorkspaceSizeInBytes(args, config);
        at::Tensor workspace_fc1 = at::detail::empty_cuda(
            {std::get<0>(workspace_sizes)}, at::ScalarType::Char, hidden_states.device(), std::nullopt);
        workspace.bmm1_workspace = workspace_fc1.data_ptr();

        auto const& stream = at::cuda::getCurrentCUDAStream(hidden_states.get_device());
        moe_runner.runPermuteGemm1(args, workspace, hidden_states.get_device(), stream, config);
        return {gemm1_output, gemm1_output_scale};
    }

    [[nodiscard]] std::vector<at::Tensor> runActivation(at::Tensor const& gemm1_output,
        at::Tensor const& gemm1_output_scale, at::Tensor const& expanded_idx_to_permuted_idx,
        at::Tensor const& total_num_padded_tokens, int64_t top_k, int64_t num_tokens, int64_t intermediate_size,
        std::vector<int64_t> tile_config_pair, std::optional<double> const& gemm1_clamp_limit = std::nullopt)
    {
        TORCH_CHECK(tile_config_pair.size() == 2, "tile_config_pair must contain [tileN, config].");
        int64_t const tileN = tile_config_pair[0];
        TORCH_CHECK(tileN != -1, "run_activation expects a resolved tile_config_pair.");

        auto args = makeFp8BlockScaleMoeArgs(
            gemm1_output, 0, top_k, std::nullopt, std::nullopt, intermediate_size, 0, 0, std::nullopt);
        args.num_tokens = static_cast<int32_t>(num_tokens);
        args.hidden_size = 0;
        args.do_finalize = false;
        if (gemm1_clamp_limit.has_value())
        {
            args.gemm1_clamp_limit_value = static_cast<float>(gemm1_clamp_limit.value());
            args.has_gemm1_clamp_limit_value = true;
        ...
        at::Tensor activation_output = at::detail::empty_cuda({gemm1_output.sizes()[0], intermediate_size},
            at::ScalarType::Float8_e4m3fn, gemm1_output.device(), std::nullopt);
        at::Tensor activation_output_scale = at::detail::empty_cuda({intermediate_size / 128, gemm1_output.sizes()[0]},
            at::ScalarType::Float, gemm1_output.device(), std::nullopt);
        ...
        auto const& stream = at::cuda::getCurrentCUDAStream(gemm1_output.get_device());
        mRunners.at(tileN)->runActivation(args, workspace, stream);
        return {activation_output, activation_output_scale};
    }

    [[nodiscard]] at::Tensor runGemm2(at::Tensor const& activation_output, at::Tensor const& activation_output_scale,
        at::Tensor const& gemm2_weights, at::Tensor const& gemm2_weights_scale,
        at::Tensor const& total_num_padded_tokens, at::Tensor const& cta_idx_xy_to_batch_idx,
        at::Tensor const& cta_idx_xy_to_mn_limit, at::Tensor const& num_non_exiting_ctas, int64_t num_experts,
        int64_t top_k, int64_t num_tokens, int64_t hidden_size, int64_t intermediate_size, int64_t local_num_experts,
        std::vector<int64_t> tile_config_pair)
    {
        auto const [tileN, config] = resolveTileConfig(
            std::move(tile_config_pair), top_k, hidden_size, intermediate_size, local_num_experts, num_tokens);

        auto args = makeFp8BlockScaleMoeArgs(activation_output, num_experts, top_k, std::nullopt, std::nullopt,
            intermediate_size, 0, local_num_experts, std::nullopt);
        args.num_tokens = static_cast<int32_t>(num_tokens);
        args.hidden_size = static_cast<int32_t>(hidden_size);
        args.gemm2_weights = gemm2_weights.data_ptr();
        args.gemm2_weights_scale = gemm2_weights_scale.data_ptr<float>();

        auto const max_num_padded_tokens_gemm2 = getMaxNumPaddedTokensGemm2(args, tileN);
        at::Tensor gemm2_output = at::detail::empty_cuda({max_num_padded_tokens_gemm2, hidden_size},
            at::ScalarType::BFloat16, activation_output.device(), std::nullopt);
        ...
        auto const workspace_sizes = moe_runner.getWorkspaceSizeInBytes(args, config);
        at::Tensor workspace_fc2 = at::detail::empty_cuda(
            {std::get<1>(workspace_sizes)}, at::ScalarType::Char, activation_output.device(), std::nullopt);
        workspace.bmm2_workspace = workspace_fc2.data_ptr();

        auto const& stream = at::cuda::getCurrentCUDAStream(activation_output.get_device());
        moe_runner.runGemm2(args, workspace, activation_output.get_device(), stream, config);
        return gemm2_output;
    }

    [[nodiscard]] at::Tensor runFinalize(at::Tensor const& gemm2_output, at::Tensor const& expert_weights,
        at::Tensor const& expanded_idx_to_permuted_idx, at::Tensor const& total_num_padded_tokens, int64_t num_experts,
        int64_t top_k, int64_t num_tokens, int64_t hidden_size, std::vector<int64_t> tile_config_pair,
        std::optional<at::Tensor> const& output = std::nullopt)
    {
        TORCH_CHECK(tile_config_pair.size() == 2, "tile_config_pair must contain [tileN, config].");
        int64_t const tileN = tile_config_pair[0];
        TORCH_CHECK(tileN != -1, "run_finalize expects a resolved tile_config_pair.");

        auto args = makeFp8BlockScaleMoeArgs(
            gemm2_output, num_experts, top_k, std::nullopt, std::nullopt, 0, 0, 0, std::nullopt);
        args.num_tokens = static_cast<int32_t>(num_tokens);
        args.hidden_size = static_cast<int32_t>(hidden_size);
        ...
        if (output.has_value())
        {
            final_output = output.value();
        }
        else
        {
            final_output = at::detail::empty_cuda(
                {num_tokens, hidden_size}, at::ScalarType::BFloat16, gemm2_output.device(), std::nullopt);
        }
        ...
        auto const& stream = at::cuda::getCurrentCUDAStream(gemm2_output.get_device());
        mRunners.at(tileN)->runFinalize(args, workspace, stream);
        return final_output;
    }
...
private:
    ...
    [[nodiscard]] std::tuple<int64_t, int64_t> resolveTileConfig(std::vector<int64_t> tileConfigPair, int64_t topK,
        int64_t hiddenSize, int64_t intermediateSize, int64_t numLocalExperts, int64_t numTokens) const
    {
        TORCH_CHECK(tileConfigPair.size() == 2, "tileConfigPair must contain [tileN, config].");
        auto [tileN, config] = std::tie(tileConfigPair[0], tileConfigPair[1]);

        if (tileN == -1 || config == -1)
        {
            float const avg_tokens_per_expert = static_cast<float>(numTokens * topK) / numLocalExperts;
            tileN = std::clamp(nextPowerOfTwo(avg_tokens_per_expert), mSupportedTileN.front(), mSupportedTileN.back());
...
TORCH_LIBRARY_FRAGMENT(trtllm, m)
{
    m.class_<tensorrt_llm::torch_ext::FP8BlockScaleMoeRunner>("FP8BlockScaleMoERunner")
        ...
        .def("resolve_tactic", &tensorrt_llm::torch_ext::FP8BlockScaleMoeRunner::resolveTactic)
        .def("run_routing", &tensorrt_llm::torch_ext::FP8BlockScaleMoeRunner::runRouting)
        .def("run_permute_gemm1", &tensorrt_llm::torch_ext::FP8BlockScaleMoeRunner::runPermuteGemm1)
        .def("run_activation", &tensorrt_llm::torch_ext::FP8BlockScaleMoeRunner::runActivation)
        .def("run_gemm2", &tensorrt_llm::torch_ext::FP8BlockScaleMoeRunner::runGemm2)
        .def("run_finalize", &tensorrt_llm::torch_ext::FP8BlockScaleMoeRunner::runFinalize)
