...
struct TopKRedType {
    ...
    static_assert(std::is_same_v<T, float> || std::is_same_v<T, int>,
                  "Top K reduction here only specialised for float / int");
...
__forceinline__ __device__ void reduceTopK(
    ...
    RedType topK[N];
#pragma unroll
    for (int nn = 0; nn < N; ++nn) topK[nn] = RedType{value[nn], idx[nn]};
...
#pragma unroll
        for (int nn = 0; nn < N; ++nn) {
            topK[nn] = update && nn == N - 1
                ? RedType{minValue, idx[nn]}
                : update ? topK[nn + 1] : topK[nn];
        }
