...
def register_auto_model(name: str):
    """Decorator that registers a model class in MODEL_CLASS_MAPPING.

    Registration happens when Python evaluates the decorator -- which only
    occurs once the module is imported, so any new modeling_*.py must also
    be added to tensorrt_llm/_torch/models/__init__.py.

    Args:
        name: Architecture string used as the lookup key. Must match the
            string the model's HF config.json puts in architectures[0]
            (e.g. "LlamaForCausalLM", "Glm4MoeForCausalLM"); that is the key
            AutoModelForCausalLM._resolve_class will look up at load time.

    Returns:
        A decorator that adds the decorated class to MODEL_CLASS_MAPPING
