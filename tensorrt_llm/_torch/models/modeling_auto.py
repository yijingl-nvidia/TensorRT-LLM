...
``config.pretrained_config.architectures[0]`` -- e.g. ``"LlamaForCausalLM"``,
...
class AutoModelForCausalLM(Generic[TModel, TConfig]):
    ...
    def _resolve_class(config: ModelConfig) -> Optional[Type]:
        """Resolve a model class from ``config.pretrained_config.architectures[0]``
        without instantiating.

        ``config.pretrained_config`` is the parsed HF ``config.json``
        (a ``transformers.PretrainedConfig`` instance);
        ``config.pretrained_config.architectures[0]`` is the same string that
        appears under the ``"architectures"`` key in the checkpoint's
        ``config.json``.

        Looks up the architecture string in ``MODEL_CLASS_MAPPING`` and returns
        the class, or ``None`` if unregistered. Two architecture-string rewrites
        happen before the lookup:

        1. **Eagle3 detection** -- checkpoints that expose ``draft_vocab_size``
           are routed to an ``EAGLE3<Arch>`` variant. Marked as a hack pending
           cleaner first-class checkpoint tagging.
        2. **MTP draft re-routing** -- when an MoE arch (DeepSeek-V3,
           Glm4-MoE, Exaone-MoE) is paired with a spec config whose
           ``max_draft_len == 0`` (i.e. the draft pass), routing redirects to
           the shared ``MTPDraftModelForCausalLM`` class.
    ...
    def from_config(
        ...
        """Resolve and instantiate the model class for ``config``.

        Raises ``ValueError`` if the architecture string is unknown -- typically
        because the matching ``modeling_*.py`` is not imported in
        ``models/__init__.py`` (decorator never ran), or the HF ``config.json``
        names an architecture TRT-LLM does not yet support.

        For ``DecoderModelForCausalLM`` subclasses, weight creation is deferred
        out of ``__init__`` (``skip_create_weights_in_init = True``); the
        executor calls into the model later to materialize weights once parallel
        layout and quant config are finalized.
        """
