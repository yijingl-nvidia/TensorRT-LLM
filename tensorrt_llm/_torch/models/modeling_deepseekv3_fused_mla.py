            ...
            if num_ctx_tokens > 0:
                ...
                for chunk_start in range(0, num_ctx_tokens, chunk_size):
                    ...
                    attn_out_latent[chunk_slice] = torch.ops.trtllm.dsv3_fused_mla_generation(
                        ...
                        q_nope=q_nope[chunk_slice],
                        k_b_proj_trans=self.k_b_proj_trans,
                        q_pe=q_pe[chunk_slice],
                        latent_cache=latent_cache[chunk_slice],
                        rotary_cos_sin=self.mqa.rotary_cos_sin,
                        sequence_length=sequence_length,
                        kv_cache_block_offsets=attn_metadata.kv_cache_block_offsets,
                        ...
                        topk_indices=topk_indices[chunk_slice],
                        topk_indices_pool=topk_indices_pool[chunk_slice],
                        kv_cache_pool=kv_cache_pool,
                        kv_scale_orig_quant=self.mqa.kv_scale_orig_quant,
                        kv_scale_quant_orig=self.mqa.kv_scale_quant_orig,
                        quant_q_buffer=quant_q_buffer_chunk,
                        ...
                        quant_mode=int(self.mqa.quant_mode),
                    ...
                    for chunk_start in range(0, tokens_per_generation_seq, chunk_size):
                        ...
                        attn_out_latent[generation_chunk_slice] = (
                            torch.ops.trtllm.dsv3_fused_mla_generation(
                                fused_q=fused_q_chunk,
                                q_nope=q_nope[generation_chunk_slice],
                                k_b_proj_trans=self.k_b_proj_trans,
                                q_pe=q_pe[generation_chunk_slice],
                                latent_cache=latent_cache[generation_chunk_slice],
                                rotary_cos_sin=self.mqa.rotary_cos_sin,
                                sequence_length=generation_sequence_length,
                                kv_cache_block_offsets=generation_kv_cache_block_offsets,
                                host_kv_cache_pool_pointers=(
                                    attn_metadata.kv_cache_manager.kv_cache_pool_pointers
                                ),
                                host_kv_cache_pool_mapping=(
                                ...
                                topk_indices=topk_indices[generation_chunk_slice],
                                topk_indices_pool=topk_indices_pool[generation_pool_chunk_slice],
                                kv_cache_pool=kv_cache_pool,
                                kv_scale_orig_quant=self.mqa.kv_scale_orig_quant,
                                kv_scale_quant_orig=self.mqa.kv_scale_quant_orig,
                                quant_q_buffer=quant_q_buffer_chunk,
