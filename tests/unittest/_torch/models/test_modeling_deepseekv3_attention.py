...
from tensorrt_llm._torch.models.modeling_deepseekv3_mla_pytorch import dsv3_mla_decode_pytorch
...
def _custom_decode_rope_generation(
    fused_q: torch.Tensor,
    q_pe: torch.Tensor,
    latent_cache: torch.Tensor,
    attention: AttentionBackend,
...
def _get_decode_current_kv_pool_rows(
    attention: AttentionBackend,
    metadata: AttentionMetadata,
    device: torch.device,
    context_sequence_length: int,
) -> tuple[torch.Tensor, torch.Tensor]:
    current_local_indices = torch.arange(
        context_sequence_length,
        context_sequence_length + _DECODE_NUM_TOKENS,
        dtype=torch.int32,
        device=device,
    ).unsqueeze(1)
    current_pool_indices, kv_cache_pool = transform_local_topk_and_prepare_pool_view(
        current_local_indices,
        metadata,
        layer_idx=attention.get_local_layer_idx(metadata),
        is_generation=True,
    )
    return current_pool_indices[:, 0].to(torch.long), kv_cache_pool
...
def test_glm5_fp8_decode_rope_generation_custom_op_matches_trtllm() -> None:
    _require_glm5_context_attention_runtime()
    device = torch.device("cuda")
    ...
    _, q_pe_expected = q_for_split.clone().split([_QK_NOPE_HEAD_DIM, _QK_ROPE_HEAD_DIM], dim=-1)
    _, q_pe_actual = q_for_split.clone().split([_QK_NOPE_HEAD_DIM, _QK_ROPE_HEAD_DIM], dim=-1)
    assert q_pe_actual.stride() == (
    ...
    expected_mla_bmm1_scale = torch.empty(2, dtype=torch.float32, device=device)
    expected_mla_bmm2_scale = torch.empty(1, dtype=torch.float32, device=device)
    expected_quant_q_buffer = torch.empty(
        _DECODE_NUM_TOKENS,
        _LOCAL_NUM_HEADS,
        _HEAD_DIM,
        dtype=torch.uint8,
    ...
    actual_mla_bmm1_scale = torch.empty_like(expected_mla_bmm1_scale)
    actual_mla_bmm2_scale = torch.empty_like(expected_mla_bmm2_scale)
    actual_quant_q_buffer = torch.empty_like(expected_quant_q_buffer)
    ...
    with torch.inference_mode():
        attention.mla_rope_generation(
            ...
            expected_mla_bmm1_scale,
            expected_mla_bmm2_scale,
            expected_quant_q_buffer,
        )
        current_pool_rows, kv_cache_pool = _get_decode_current_kv_pool_rows(
            attention,
            metadata,
            device,
            context_sequence_length=_CONTEXT_SEQUENCE_LENGTH,
        )
        expected_kv_rows = kv_cache_pool[current_pool_rows, 0, :].view(torch.uint8).clone()

        sentinel_rows = torch.full(
            (_DECODE_NUM_TOKENS, 1, _HEAD_DIM),
            0xA5,
            dtype=torch.uint8,
            device=device,
        )
        kv_cache_pool.view(torch.uint8).index_copy_(0, current_pool_rows, sentinel_rows)
        _custom_decode_rope_generation(
            fused_q.clone(),
            q_pe_actual,
            latent_cache.clone(),
            attention,
            metadata,
            actual_mla_bmm1_scale,
            actual_mla_bmm2_scale,
            actual_quant_q_buffer,
        )
        torch.cuda.synchronize()
        actual_kv_rows = kv_cache_pool[current_pool_rows, 0, :].view(torch.uint8).clone()

    torch.testing.assert_close(actual_quant_q_buffer, expected_quant_q_buffer, rtol=0, atol=0)
    torch.testing.assert_close(actual_mla_bmm1_scale, expected_mla_bmm1_scale, rtol=0, atol=0)
    torch.testing.assert_close(actual_mla_bmm2_scale, expected_mla_bmm2_scale, rtol=0, atol=0)
    torch.testing.assert_close(actual_kv_rows, expected_kv_rows, rtol=0, atol=0)
